...
/** Formulário único de Viagem em dois modos: criação (`cadastrarViagem`) e edição
 * (`editarViagem`, US-109 Cenário 4). O modo é dado por `viagemEmEdicao`: quando presente,
 * o submit chama a Server Action de edição passando `viagemId`, o título/botão mudam e
 * aparece "Cancelar". Os vínculos Proposta/Versão/Meta são congelados no backend — aqui só
 * expomos descrição, quantitativos, custos unitários e contas analíticas. O componente pai
 * remonta este form (via `key={chaveFormulario}`) sempre que troca de modo ou de viagem. */
function ViagemForm({
  versaoId,
  contasAnaliticas,
  readOnly,
  ...
  viagemEmEdicao: ViagemResultado | null;
  onSalva: (resultado: ViagemResultado) => void;
  ...
  function salvar() {
    ...
    startTransition(async () => {
      ...
      if (!viagemEmEdicao) setDescricao('');
      onSalva(resposta.dados);
    });
  }
      ...
      <div className="flex items-center gap-2">
        ...
        {editando && (
          <button
            ...
            disabled={pending}
            className="rounded border border-gray-200 px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-50 disabled:opacity-50"
          >
            Cancelar
          </button>
        )}
 ...
 * bordas suaves, slate-* para hierarquia neutra).
 * - "Copiar" pré-preenche o formulário (`chaveFormulario` força o React a remontar o form com os
 *   novos valores iniciais) para cadastrar uma NOVA Viagem a partir de uma existente.
 * - "Editar" carrega a Viagem no mesmo formulário em modo edição (`editarViagem`), sem sair da tela. */
export function ViagemPanel({
  ...
  const [viagemEmEdicaoId, setViagemEmEdicaoId] = useState<string | null>(null);
  const formularioRef = useRef<HTMLDivElement>(null);

  const viagemEmEdicao = useMemo(
    () => viagens.find((v) => v.id === viagemEmEdicaoId) ?? null,
    [viagens, viagemEmEdicaoId],
  );
  ...
  function excluir(viagemId: string) {
    startTransition(async () => {
      ...
      if (resposta.sucesso) {
        setViagens((atual) => atual.filter((v) => v.id !== viagemId));
        if (viagemEmEdicaoId === viagemId) voltarParaCriacao();
      }
    });
  }

  // ADR-036 — dashboard deriva do state `viagens` já existente (useMemo), recalcula sozinho
  // ao criar/editar/copiar/excluir Viagem, sem código de sincronização adicional.
  ...
  function editar(viagem: ViagemResultado) {
    setValoresFormulario(valoresDe(viagem, viagem.descricao));
    setViagemEmEdicaoId(viagem.id);
    irParaFormulario();
  }

  function aoSalvar(v: ViagemResultado) {
    setViagens((atual) => (atual.some((x) => x.id === v.id) ? atual.map((x) => (x.id === v.id ? v : x)) : [...atual, v]));
    voltarParaCriacao();
  }
    ...
    <div className="flex flex-col gap-4 rounded-xl bg-slate-50 p-4 md:p-6">
      ...
      <div ref={formularioRef}>
        <ViagemForm
          ...
          readOnly={readOnly}
          valoresIniciais={valoresFormulario}
          viagemEmEdicao={viagemEmEdicao}
          onSalva={aoSalvar}
          onCancelarEdicao={voltarParaCriacao}
        />
      </div>
      {viagens.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {viagens.map((viagem) => {
            const emEdicao = viagem.id === viagemEmEdicaoId;
            return (
              <li
                key={viagem.id}
                className={`flex items-center justify-between gap-2 rounded-lg border px-3 py-2.5 text-sm shadow-sm ${
                  emEdicao ? 'border-blue-300 bg-blue-50' : 'border-gray-100 bg-white'
                }`}
              >
                <span>
