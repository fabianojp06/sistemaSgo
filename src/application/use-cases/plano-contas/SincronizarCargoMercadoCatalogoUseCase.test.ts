...
function criarDependencias() {
  ...
  const provider = { buscarCatalogoAtivo: vi.fn().mockResolvedValue([]) };
...
describe('SincronizarCargoMercadoCatalogoUseCase [ADR-047/US-139]', () => {
  it('rejeita usuário sem permissão administrativa', async () => {
    const { prisma, provider, loader, lock } = criarDependencias();
    const useCase = new SincronizarCargoMercadoCatalogoUseCase(prisma as never, provider as never, loader as never, lock as never);

    await expect(
      useCase.execute({ tenantId: 't1', usuarioId: 'u1', temPermissaoAdministrativa: false }),
    ).rejects.toThrow(AcessoNegadoSincronismoCargoMercadoCatalogoError);
  ...
  it('rejeita sincronismo concorrente quando lock não é adquirido', async () => {
    ...
    ).rejects.toThrow(SincronismoCargoMercadoCatalogoEmAndamentoError);
    expect(provider.buscarCatalogoAtivo).not.toHaveBeenCalled();
  });

  it('sincroniza, grava auditoria SYNC_CARGO_MERCADO_CATALOGO e sempre libera o lock (Cenário 4)', async () => {
    const { prisma, provider, loader, lock } = criarDependencias();
    const useCase = new SincronizarCargoMercadoCatalogoUseCase(prisma as never, provider as never, loader as never, lock as never);
    ...
    expect(prisma.historicoOperacao.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ tipoOperacao: 'SYNC_CARGO_MERCADO_CATALOGO' }) }),
    );
