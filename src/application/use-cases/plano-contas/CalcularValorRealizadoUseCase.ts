 ...
 * da Proposta — cálculo extraído para `ValorRealizadoService`, reusado
 * também pelo dashboard da guia Valor Orçado (US-118).
 */
export class CalcularValorRealizadoUseCase {
  ...
  async execute(
    ...
    const valorRealizadoPorConta = await this.valorRealizadoService.somarPorContaAnalitica(tenantId, versaoId);
