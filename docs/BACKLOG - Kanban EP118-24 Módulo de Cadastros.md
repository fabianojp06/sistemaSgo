...
| US-141 | Seletor de Município (IBGE) na Localidade da Viagem | PR #17 (`54d5298`, 2026-09-02). Catálogo IBGE embutido (5.571 municípios, `kelvins/municipios-brasileiros` MIT, sem sync); `Viagem` +`municipioIbge`/snapshot nome-uf/lat-long; `descricao` → "Motivo/Complemento" opcional; combobox client-side. ADR-048. Migration aplicada em produção. Follow-up leve pendente (ver `docs/US-141 ...md`) |
...
|---|---|---|---|
| 1 | **US-142 — Cronograma de Desembolso por Parcelas (ANEXO 9)** [Módulo Orçamentário] | Substitui a grade mensal (US-138) por tabela de parcelas T1..Tn (layout do PDF real ANEXO 9 do TP PAME-RJ/CTCEA/2025): sub-linha "Evento Tn Meta 01", subtotais anuais + coluna "Valor Acumulado por Ano do TP" + total geral. Calendário configurável na Proposta (`parcelasPorAno`/`mesInicialRepasse`); parcela de entrada; período antecipado. Refinada (`docs/US-142 ...md`) + **ADR-049 aceito** — **pronta para o `fullstack-dev`**. Migration aditiva (2 campos + CHECK). Muda motor de cálculo + tela + form de Proposta → branch + PR + `/code-review`; migration aplicada junto do merge. CD-06 (Viagem sem data) → US-143 futura. | G (~3–4 dias) |
| 2 | **US-144 — Motor de Cálculo Automático de Imposto (Conta Analítica)** [Orçamentário] [MVP do épico Impostos] | Botão "Gerar Impostos da Versão": calcula `imposto = custo bruto da conta × alíquota%` (sem cascata), grava linhas `modoValor=CALCULADO` sem tocar as `DECLARADO` (manuais/reajuste). Migration aditiva (`modoValor`, `valorBaseSnapshot`, `AliquotaImpostoParametro.categoria`). Congelamento + imunidade TP + aviso de "stale". `docs/US-144 ...md` + ADR-050. Migration junto do merge. | G |
| 3 | **US-145 — Imposto sobre Conta Sintética** [Orçamentário] | `contaId` aceita sintética; imposto aplicado após o bottom-up direto no valor agregado (C1). Flag `temImpostoDireto` + nota "não é soma pura das filhas". Depende de US-144. `docs/US-145 ...md`. | M |
| 4 | **US-146 — Exibir "Custo" e "Custo c/ Impostos"** [Orçamentário] | Badge do Semáforo e dashboard US-118 mostram os 2 valores. % e cor do Semáforo e Valor Global **inalterados** (seguem "com imposto"). Sem migration. Depende de US-144. `docs/US-146 ...md`. | M |
| 5 | **ÉPICO — Aplicação Automática de Impostos sobre Contas** [Orçamentário] | Guarda-chuva de US-144/145/146. Descoberta (`docs/EPICO ...md`) + **ADR-050 aceito** (substitui ADR-039). US-147 (separar tributo de índice de reajuste) = dívida técnica opcional. | épico ~2 semanas |
