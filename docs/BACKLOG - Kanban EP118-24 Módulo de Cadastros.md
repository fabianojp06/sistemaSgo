...
| 2 | **ÉPICO — Aplicação Automática de Impostos sobre Contas** [Orçamentário] | Cálculo automático `base × Σ alíquotas` por Proposta × Versão × Conta — analíticas **e sintéticas** (C1). Congela pós-oficialização (D1). Exibe "sem imposto"/"com imposto". Descoberta (`docs/EPICO ...md`) + **ADR-050 aceito** (substitui ADR-039): `RateioImpostoGrade` +`modoValor`/`valorBaseSnapshot`, botão "Gerar Impostos", grandfather dos dados atuais, reajuste (ADR-040) intacto. **Pronto para AN/PO escrever US-144/145/146.** Migration aditiva junto do merge. | US-144 = G; épico ~2 semanas |
