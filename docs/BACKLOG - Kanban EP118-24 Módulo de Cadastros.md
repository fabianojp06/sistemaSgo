...
|---|---|---|---|
| 1 | **US-142 — Cronograma de Desembolso por Parcelas (APÊNDICE J)** [Módulo Orçamentário] | Substitui a grade mensal (US-138) por tabela de parcelas T1..Tn com sub-linha "Meta Única", subtotais anuais + total geral. Calendário de repasse configurável na Proposta (`parcelasPorAno`/`mesInicialRepasse`). Refinada (`docs/US-142 ...md`) + **ADR-049 aceito** (`docs/ADR-049 ...md`) — **pronta para o `fullstack-dev`**. Migration aditiva (2 campos em `Proposta` + CHECK). Muda motor de cálculo + tela + form de Proposta → branch + PR + `/code-review`; migration aplicada junto do merge. CD-06 (Viagem sem data) → US-143 futura. | G (~3–4 dias) |
