...
|---|---|---|---|
| 1 | **US-142 — Cronograma de Desembolso por Parcelas (APÊNDICE J)** [Módulo Orçamentário] | Substitui a grade mensal atual (US-138) por tabela de parcelas T1..Tn (3/ano jan/mai/set, DERIVADAS do custo), sub-linhas por Meta, subtotais anuais + total geral, no layout do documento-alvo do concedente. Refinada (`docs/US-142 ...md`) — **bloqueada por ADR**: 7 GAPs (regra de repasse, texto "Etapa M", 1ª parcela, rateio por Meta p/ CONSOLIDADA + imposto, "valor repassado 12 meses", Viagem sem data, exportação). `techlead-fsg` faz o ADR antes de codar. Muda motor de cálculo + tela → branch + PR + `/code-review`. | G (~3–4 dias) |
