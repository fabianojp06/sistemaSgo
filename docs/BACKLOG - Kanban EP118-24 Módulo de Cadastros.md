...
| 2 | **ÉPICO — Aplicação Automática de Impostos sobre Contas** [Orçamentário] | Cálculo automático `base × Σ alíquotas` (base = Empregados+Viagens+Bens, sem o próprio imposto) por Proposta × Versão × Conta — **analíticas e sintéticas** (C1: ajuste direto na sintética). Congela pós-oficialização (D1). Exibir "valor sem imposto"/"com imposto". Descoberta feita (`docs/EPICO ...md`), respostas do ADR-039 colhidas. **Bloqueado:** `techlead-fsg` reescreve o ADR-039 (modelagem, gatilho de recálculo, dados manuais, impacto ADR-040). Decompõe em US-144/145/146. Conflita com ADR-027 (contaId analítica). | US-144 = G; épico ~2 semanas |
