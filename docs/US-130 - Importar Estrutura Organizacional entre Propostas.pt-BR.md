...
**Pré-requisito de UI (fechado via [ADR-041](ADR-041%20-%20Sequenciamento%20US-116-US-130%20e%20Remapeamento%20de%20Hierarquia.pt-BR.md))**: hoje não existe tela de Estrutura Funcional (backend pronto desde US-106, UI ainda no backlog como US-116 — `docs/BACKLOG - Kanban EP118-24 Módulo de Cadastros.md`). A ação "Importar de outra Proposta" é um botão/fluxo dentro da tela de US-116 (`/propostas/{id}/estrutura`). **Sequenciamento decidido: US-116 primeiro, em branch/PR próprio; US-130 só começa depois de US-116 mergeada na master** — não entram no mesmo PR (ver ADR-041 para a justificativa completa e o desenho técnico do remapeamento de hierarquia self-relation).
